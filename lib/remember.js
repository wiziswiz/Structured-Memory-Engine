...
const { gateCheck, containsInjection } = require('./quality-gate');
...
function remember(workspace, content, { tag = 'fact', date = null, qualityGate = null } = {}) {
  ...
  const injectionMatch = containsInjection(sanitized);
  if (injectionMatch) {
    const msg = `Injection pattern detected: "${injectionMatch}" in content: "${sanitized.slice(0, 100)}..."`;
    console.error(`[SME:INJECTION_BLOCKED] ${msg}`);
    return { filePath: null, created: false, line: null, skipped: true, injectionBlocked: true, reason: msg };
  }
