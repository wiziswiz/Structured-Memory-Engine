...
const { expandEntitiesWithCooccurrence, extractQueryEntities, getEntityIndexMap } = require('./entities');
...
function cilScore(chunk, nowMs, opts) {
  ...
  const overrides = {};
  if (opts.recencyBoostDays) overrides.recencyHalfLifeDays = opts.recencyBoostDays;
  // v10 F5: thread query entities for entityMention signal
  if (opts.queryEntities && opts.queryEntities.length > 0 && opts.entityIndexMap) {
    overrides.queryEntities = opts.queryEntities;
    overrides.entityIndexMap = opts.entityIndexMap;
  }
  return computeScore(chunk, nowMs, profile, Object.keys(overrides).length ? overrides : undefined);
}
...
function formatContext(selectedChunks, contradictions, opts = {}) {
  if (selectedChunks.length === 0) return '';

  const stalenessWarningDays = opts.stalenessWarningDays || 30;

  let out = '## Recalled Context\nStructured memories retrieved by relevance. Source citations included.\n\n';
  const fileRenderCount = {};

  for (const chunk of selectedChunks) {
    const fileKey = chunk.filePath || '';
    const count = fileRenderCount[fileKey] || 0;
    ...
    // v10.1: staleness annotation — warn when recalled chunk is old
    let stalenessLabel = '';
    if (chunk.date) {
      const daysOld = Math.floor((Date.now() - new Date(chunk.date).getTime()) / 86400000);
      if (daysOld >= stalenessWarningDays) {
        stalenessLabel = ` ⏳ ${daysOld}d ago — verify before acting`;
      }
    }
    ...
    if (count === 0) {
      ...
      out += `\n  ↳ ${source}${typeLabel}${confLabel}${age}${stalenessLabel}\n`;
    } else {
      // Secondary: collapsed one-liner
      const preview = chunk.content.slice(0, 80).replace(/\n/g, ' ');
      out += `- (+${count} related) "${preview}..."\n  ↳ ${source}${typeLabel}${age}${stalenessLabel}\n`;
    }
    fileRenderCount[fileKey] = count + 1;
  }
...
/**
 * v10.1: Domain diversity enforcement.
 * Ensures each active domain (health, crypto, work, etc.) has at least one chunk
 * in the top-N results. Without this, a dominant domain monopolizes the context window.
 *
 * Strategy: look at the top maxChunks results. For each domain with chunks in the
 * full set but zero representation in top-N, promote the highest-scoring chunk from
 * that domain into the top-N by displacing the lowest-scoring chunk from the most
 * over-represented domain. Returns the full array re-sorted so caller can slice normally.
 */
function enforceDomainDiversity(results, maxChunks) {
  if (results.length <= 1) return results;

  const cap = Math.min(maxChunks, results.length);

  // Count domains in top-N
  const topDomains = {};
  for (let i = 0; i < cap; i++) {
    const d = results[i].domain || 'general';
    topDomains[d] = (topDomains[d] || 0) + 1;
  }

  // Find domains present below top-N but missing from it
  const promotions = []; // { poolIdx, domain }
  const seenMissing = new Set();
  for (let i = cap; i < results.length; i++) {
    const d = results[i].domain || 'general';
    if (d === 'general') continue; // don't force-inject generic chunks
    if (topDomains[d]) continue; // already represented in top-N
    if (seenMissing.has(d)) continue; // already found best candidate for this domain
    seenMissing.add(d);
    promotions.push({ poolIdx: i, domain: d });
  }

  if (promotions.length === 0) return results;

  // Make a mutable copy of the array
  const out = results.slice();

  for (const { poolIdx, domain } of promotions) {
    // Find the most over-represented domain in top-N (most chunks, >1)
    let maxDomain = null;
    let maxCount = 1;
    for (const [d, count] of Object.entries(topDomains)) {
      if (count > maxCount) {
        maxDomain = d;
        maxCount = count;
      }
    ...
    if (!maxDomain) continue; // all domains at 1 — can't displace

    // Find lowest-scoring chunk from over-represented domain in top-N
    let worstIdx = -1;
    let worstScore = Infinity;
    for (let i = 0; i < cap; i++) {
      const d = out[i].domain || 'general';
      if (d === maxDomain && (out[i]._cilScore || 0) < worstScore) {
        worstScore = out[i]._cilScore || 0;
        worstIdx = i;
      }
    }

    if (worstIdx >= 0) {
      // Swap the pool chunk into top-N position
      const tmp = out[worstIdx];
      out[worstIdx] = out[poolIdx];
      out[poolIdx] = tmp;
      topDomains[maxDomain]--;
      topDomains[domain] = 1;
    }
  }

  // Re-sort top-N by score so highest-scoring chunks come first
  const topSlice = out.slice(0, cap);
  topSlice.sort((a, b) => (b._cilScore || 0) - (a._cilScore || 0));
  for (let i = 0; i < cap; i++) out[i] = topSlice[i];
...
function getRelevantContext(db, message, opts = {}) {
  ...
  const {
    ...
    minCilScore: minCilScoreOpt = 0.25,
    fileWeights = null,
    recallGating = null,
    skipGate = false,
  ...
  // v10 F3: Recall Gate — skip trivial messages (acks, math, system cmds) with
  // zero DB access. Callers can bypass via skipGate: true for explicit calls
  // where the user/agent already chose to query.
  if (!skipGate && (!recallGating || recallGating.enabled !== false)) {
    const gate = shouldRecall(message, { recallGating: recallGating || undefined });
    if (!gate.shouldRecall) {
      return { text: '', chunks: [], tokenEstimate: 0, gated: true, gateReason: gate.reason };
    }
  }
  ...
  // v10 F5: Extract query entities early — F1 orchestrator uses them
  // to decide whether requireEntities strategies fire, and scoring uses
  // them for the entityMention signal.
  let queryEntities = null;
  let entityIndexMap = null;
  try {
    queryEntities = extractQueryEntities(db, message);
    if (queryEntities && queryEntities.length > 0) {
      entityIndexMap = getEntityIndexMap(db, queryEntities);
    }
  } catch (_) { /* entity_index may not exist yet — non-fatal */ }

  // v10 F1: Orchestrator — classify query intent → pick profile.
  // Explicit opts.recallProfile still wins.
  let strategy = null;
  if (!opts.recallProfile && opts.orchestrator !== false) {
    strategy = classifyStrategy(message, queryEntities || []);
  }
  ...
  for (const r of results) {
    r._cilScore = cilScore(r, nowMs, {
      recencyBoostDays: effectiveRecencyBoost,
  ...
  // v10.1: Domain diversity — ensure each active domain has at least one chunk represented.
  // Without this, 2 weeks of crypto activity buries all health/personal context.
  if (diversityConfig.domainDiversity !== false && results.length > 0) {
    results = enforceDomainDiversity(results, maxChunks);
  }
  ...
  const formatOpts = { stalenessWarningDays };
  let text = formatContext(budgeted, contradictions, formatOpts);
  ...
  while (tokenEstimate > maxTokens && budgeted.length > 1) {
    ...
    text = formatContext(budgeted, contradictions, formatOpts);
...
module.exports = { getRelevantContext, extractQueryTerms, cilScore, budgetChunks, formatContext, enforceDomainDiversity, invalidateEntityCache, detectQueryIntent, isRuleChunk, applyRulePenalty };
