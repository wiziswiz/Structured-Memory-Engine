...
function decayConfidence(db, { dryRun = false, config = null } = {}) {
  ...
  // v10 F4: protected chunks skip decay entirely (user-curated, no auto drift)
  const rows = db.prepare("SELECT id, heading, chunk_type, confidence, last_accessed, created_at, content_updated_at, priority FROM chunks WHERE confidence > 0 AND chunk_type != ? AND (protected IS NULL OR protected = 0)").all('confirmed');
  const details = [];
  const updates = [];
...
function markStale(db, { dryRun = false } = {}) {
  const now = Date.now();
  // v10 F4: protected chunks are never marked stale
  const rows = db.prepare('SELECT id, heading, confidence, created_at FROM chunks WHERE stale = 0 AND (protected IS NULL OR protected = 0)').all();
  const details = [];
  const updates = [];
...
function pruneStale(db, { dryRun = false } = {}) {
  ...
  // v10 F4: protected chunks are never archived by pruning
  const rows = db.prepare('SELECT * FROM chunks WHERE stale = 1 AND (protected IS NULL OR protected = 0)').all();
  const details = [];
  const toArchive = [];
...
function valueAssessment(db, { dryRun = false, mode = 'apply' } = {}) {
  ...
  // v10 F4: protected chunks are excluded from value assessment decay/archival
  const rows = db.prepare('SELECT id, content, chunk_type, confidence, file_path, created_at FROM chunks WHERE stale = 0 AND (protected IS NULL OR protected = 0)').all();
...
/**
 * v10.1: File freshness watchdog — flag indexed files that haven't been updated
 * within a configurable threshold. Catches silently rotting context bundles
 * like health.md going 41 days without an update.
 */
function checkFileFreshness(db, { config = null } = {}) {
  const alertDays = (config && config.reflect && config.reflect.freshnessAlertDays) || 14;
  const now = Date.now();
  const thresholdMs = alertDays * 86400000;

  let rows;
  try {
    rows = db.prepare('SELECT file_path, mtime_ms FROM files').all();
  } catch (_) {
    return { staleFiles: [], alertDays };
  }

  const staleFiles = [];
  for (const row of rows) {
    const age = now - row.mtime_ms;
    if (age > thresholdMs) {
      staleFiles.push({
        filePath: row.file_path,
        daysStale: Math.floor(age / 86400000),
        lastModified: new Date(row.mtime_ms).toISOString().split('T')[0],
      });
    }
  }

  // Sort by staleness (most stale first)
  staleFiles.sort((a, b) => b.daysStale - a.daysStale);

  return { staleFiles, alertDays };
}

/**
 * v10.1: Cross-file entity-scoped contradiction detection.
 * Catches contradictions like "Bromantane active" in health.md vs "Bromantane PAUSED"
 * in MEMORY.md — different headings, different files, same entity.
 * Complements the existing heading-based detectContradictions.
 */
function detectCrossFileContradictions(db, { dryRun = false, config = null } = {}) {
  const minShared = (config && config.reflect && config.reflect.contradictionMinSharedTerms != null)
    ? config.reflect.contradictionMinSharedTerms : 3;

  // Load entity index
  let entityRows;
  try {
    entityRows = db.prepare('SELECT entity, chunk_ids FROM entity_index WHERE mention_count >= 2').all();
  } catch (_) {
    return { found: 0, newFlags: 0, details: [] };
  }

  // Load existing contradiction pairs to skip
  const existing = new Set();
  const existingRows = db.prepare('SELECT chunk_id_old, chunk_id_new FROM contradictions').all();
  for (const e of existingRows) existing.add(`${e.chunk_id_old}:${e.chunk_id_new}`);
  ...
  function getChunk(id) {
    ...
    const row = db.prepare('SELECT id, heading, content, file_path, created_at, domain FROM chunks WHERE id = ? AND stale = 0').get(id);
    chunkCache.set(id, row || null);
    return row || null;
  }
  ...
  for (const entityRow of entityRows) {
    let chunkIds;
    try {
      chunkIds = JSON.parse(entityRow.chunk_ids || '[]');
    } catch (_) { continue; }

    if (chunkIds.length < 2 || chunkIds.length > 50) continue;

    // Load chunks and group by file
    const chunks = chunkIds.map(id => getChunk(id)).filter(Boolean);
    const fileGroups = {};
    for (const c of chunks) {
      if (!fileGroups[c.file_path]) fileGroups[c.file_path] = [];
      fileGroups[c.file_path].push(c);
    }

    const files = Object.keys(fileGroups);
    if (files.length < 2) continue; // same file — heading-based detector handles this

    // Compare chunks across different files
    for (let fi = 0; fi < files.length; fi++) {
      for (let fj = fi + 1; fj < files.length; fj++) {
        const groupA = fileGroups[files[fi]];
        const groupB = fileGroups[files[fj]];
        ...
        for (const a of groupA) {
          for (const b of groupB) {
            ...
            // Domain gate: skip cross-domain comparisons
            const domainA = a.domain || 'general';
            const domainB = b.domain || 'general';
            if (domainA !== 'general' && domainB !== 'general' && domainA !== domainB) continue;
            ...
            const termsBSet = new Set(termsB);
            const shared = [...new Set(termsA.filter(t => termsBSet.has(t)))];
            if (shared.length < minShared) continue;

            // Divergence check — use deduplicated sizes for consistent comparison
            const uniqueA = new Set(termsA).size;
            const uniqueB = termsBSet.size;
            const smallerSize = Math.min(uniqueA, uniqueB);
            if (smallerSize > 0 && shared.length / smallerSize > 0.8) continue;
            ...
            const aNear = negationNearSharedTerm(a.content, shared, 8);
            const bNear = negationNearSharedTerm(b.content, shared, 8);
            if (!aNear && !bNear) continue;

            const reason = `Cross-file [${entityRow.entity}]: shared terms: ${shared.slice(0, 5).join(', ')}; negation detected`;
            details.push({ idOld, idNew, headingOld: a.heading, headingNew: b.heading, reason });
            inserts.push({ idOld, idNew, reason, now });
            existing.add(`${idOld}:${idNew}`);
          }
  ...
  return { found: inserts.length, newFlags: inserts.length, details };
}
...
function runReflectCycle(db, { dryRun = false, config = null, mode = null } = {}) {
  ...
  // v10.1: Cross-file entity-scoped contradiction detection
  let crossFileContradictions = { found: 0, newFlags: 0, details: [] };
  try {
    crossFileContradictions = detectCrossFileContradictions(db, { dryRun: effectiveDryRun, config });
  } catch (err) { console.debug('[sme:reflect] cross-file contradiction detection failed:', err.message); }

  // v10.1: File freshness watchdog
  let fileFreshness = { staleFiles: [], alertDays: 14 };
  try {
    fileFreshness = checkFileFreshness(db, { config });
  } catch (err) { console.debug('[sme:reflect] file freshness check failed:', err.message); }

  return { decay, reinforce, stale, contradictions, crossFileContradictions, restructure, prune, entityIndex, valueAssessment: valueAssessmentResult, retroDedup, fileFreshness, mode: effectiveMode };
}
...
module.exports = { decayConfidence, reinforceConfidence, markStale, detectContradictions, detectCrossFileContradictions, checkFileFreshness, detectSuperseded, computeChunkSimilarity, pruneStale, restoreChunk, resolveContradiction, runReflectCycle, listContradictions, extractTerms, extractDateFromPath, negationNearSharedTerm, getLastReflectTime, setLastReflectTime, valueAssessment, retroDedupScan, REVIEW_VERSION, PRIORITY_DECAY_MULTIPLIER };
