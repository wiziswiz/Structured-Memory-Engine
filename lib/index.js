...
if (!command || command === 'help' || args.help) {
  ...
  node lib/index.js query "search terms" [--limit N] [--since 7d|2w|3m|1y|2026-01-01] [--context N] [--type TYPE] [--min-confidence 0.5] [--include-stale] [--explain]
  ...
  node lib/index.js forget <id|query> [--workspace PATH]
  node lib/index.js update <id|query> "new content" [--workspace PATH]
  node lib/index.js replace <id|query> "new content" [--workspace PATH]
  node lib/index.js protect <id|query> [--workspace PATH]`);
  process.exit(0);
}
...
if (command === 'index') {
  ...
  if (!query) { console.error('Usage: node lib/index.js query "search terms" [--explain]'); process.exit(1); }
  const config = loadConfig(workspace);
  const recallResult = recall(db, query, {
    limit: args.limit, since: args.since, context: args.context || 0, workspace,
  ...
  const results = args.explain ? recallResult.results : recallResult;
  if (args.explain && recallResult.explain) {
    const ex = recallResult.explain;
    console.log(`Strategy: ${ex.strategy} (${ex.reason})`);
    console.log(`Profile:  ${ex.profile}`);
    console.log(`Weights:  ${ex.weights}`);
    console.log(`HalfLife: ${ex.halfLife}d   ConfExp: ${ex.confExponent}`);
    console.log(`Entities: [${ex.queryEntities.join(', ')}]`);
    console.log('');
  }
  if (args.json) {
...
} else if (command === 'forget' || command === 'update' || command === 'replace' || command === 'protect') {
  const target = args._[1];
  if (!target) {
    console.error(`Usage: node lib/index.js ${command} <id|query>${command === 'update' || command === 'replace' ? ' "new content"' : ''} [--workspace PATH]`);
    process.exit(1);
  }
  const content = (command === 'update' || command === 'replace') ? args._.slice(2).join(' ') : null;
  if ((command === 'update' || command === 'replace') && !content) {
    console.error(`Error: ${command} requires new content as the next argument`);
    process.exit(1);
  }
  const { executeAction, AmbiguousTargetError, TargetNotFoundError } = require('./memory-actions');
  try {
    const result = executeAction(db, { action: command, target, content, workspace });
    if (args.json) {
      console.log(JSON.stringify(result));
    } else {
      const idLabel = result.id != null ? result.id : result.oldId;
      console.log(`${command} ok: chunk ${idLabel}${result.newId ? ` → ${result.newId}` : ''}`);
    }
  } catch (err) {
    if (err instanceof AmbiguousTargetError) {
      console.error('Ambiguous target — multiple chunks match within the ambiguity delta:');
      for (const c of err.candidates) {
        console.error(`  [${c.id}] ${c.filePath}  (score ${(c.score || 0).toFixed(3)})`);
        console.error(`      ${c.content.slice(0, 100)}${c.content.length > 100 ? '...' : ''}`);
      }
      console.error('Re-run with an explicit chunk id.');
      process.exit(2);
    }
    if (err instanceof TargetNotFoundError) {
      console.error(`Target not found: ${err.target}`);
      process.exit(3);
    }
    throw err;
  }
