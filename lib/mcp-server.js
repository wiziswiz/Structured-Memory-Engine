...
const { runReflectCycle, getLastReflectTime, setLastReflectTime, listContradictions, resolveContradiction, checkFileFreshness } = require('./reflect');
...
async function handleReflect(db, args, config, workspace) {
  ...
  // v10.1: Cross-file contradictions
  if (result.crossFileContradictions && result.crossFileContradictions.newFlags > 0) {
    text += `\n  Cross-file contradictions: ${result.crossFileContradictions.newFlags} new`;
  }
  // v10.1: File freshness
  if (result.fileFreshness && result.fileFreshness.staleFiles.length > 0) {
    text += `\n  ⚠ Stale files (>${result.fileFreshness.alertDays}d): ${result.fileFreshness.staleFiles.length}`;
    for (const f of result.fileFreshness.staleFiles.slice(0, 5)) {
      text += `\n    ${f.filePath} (${f.daysStale}d)`;
    }
  }
...
function handleStatus(db, config) {
  ...
  // Alerts
  const freshness = checkFileFreshness(db, { config });
  const hasAlerts = stats.staleCount > 0 || stats.lowConfCount > 0 || freshness.staleFiles.length > 0;
  if (hasAlerts) {
    text += '\n\nAlerts:';
    ...
    if (freshness.staleFiles.length > 0) {
      text += `\n  ⚠ ${freshness.staleFiles.length} file(s) stale (>${freshness.alertDays}d since update):`;
      for (const f of freshness.staleFiles.slice(0, 10)) {
        text += `\n    ${f.filePath} (${f.daysStale}d, last: ${f.lastModified})`;
      }
      if (freshness.staleFiles.length > 10) text += `\n    ...and ${freshness.staleFiles.length - 10} more`;
    }
  }
  // Distributions
...
async function main() {
    ...
    async (args) => {
      try {
        ...
        const result = getRelevantContext(db, args.message, {
          ...
          recallGating: config.recallGating || null,
          skipGate: true, // sme_context is an explicit call — bypass the gate
        });
        if (!result.text) {
          const msg = result.gated
            ? `No context retrieved (gated: ${result.gateReason}).`
  ...
  server.tool(
    'sme_auto_context',
    `Zero-setup auto-injection. Call at the start of every user turn with the raw message. Runs a recall gate (skips acknowledgments like "ok"/"thanks", math expressions, system commands) — gated messages short-circuit without DB access. Substantive messages run the full CIL pipeline and return ranked memory context. Safe to call on every turn.`,
    {
      message: z.string().min(1).max(5000).describe('The raw user message (max 5000 chars)'),
      maxTokens: z.number().optional().describe('Token budget for injected context (default 1500)'),
      conversationContext: z.array(z.string().max(2000)).max(10).optional().describe('Recent user messages for multi-turn awareness (last 2-3 turns)'),
    },
    async (args) => {
      try {
        if (config.autoInject && config.autoInject.enabled === false) {
          return { content: [{ type: 'text', text: 'auto-inject disabled in config' }] };
        }

        let queryEmbedding = null;
        try {
          queryEmbedding = await embeddings.embed(args.message);
        } catch (err) { console.debug('[sme:mcp] auto-context embedding failed:', err.message); }

        const maxTokens = args.maxTokens
          || (config.autoInject && config.autoInject.maxTokens)
          || 1500;
        ...
        const result = getRelevantContext(db, args.message, {
          ...
          fileWeights: config.fileWeights || null,
          recallGating: config.recallGating || null,
          // skipGate: false — this tool IS the gate's primary caller
        });

        if (result.gated) {
          return { content: [{ type: 'text', text: `[sme] gated: ${result.gateReason}` }] };
        }
        if (!result.text) {
          return { content: [{ type: 'text', text: '[sme] no relevant context' }] };
        }
        return { content: [{ type: 'text', text: result.text }] };
      } catch (err) {
        log(`sme_auto_context error: ${err.message}`);
        return { content: [{ type: 'text', text: `Error: ${err.message}` }], isError: true };
      }
    }
  );
  ...
  server.tool(
    'sme_memory_action',
    `Mutate a stored memory chunk. Four actions: "update" rewrites content in-place; "replace" supersedes the old chunk with a new one (archiving the original and re-extracting entities and priority); "forget" soft-deletes (confidence=0); "protect" exempts the chunk from automatic decay and pruning. Target accepts a numeric chunk id or a query string — query-based targeting requires an unambiguous top match (refuses if top-2 are within 0.08 score delta).`,
    {
      action: z.enum(['update', 'replace', 'forget', 'protect']).describe('The mutation to apply'),
      target: z.union([z.string().min(1).max(500), z.number().int().positive()]).describe('Chunk id (number) or query string to locate the chunk'),
      content: z.string().min(1).max(5000).optional().describe('New content (required for update and replace)'),
    },
    async (args) => {
      try {
        const { executeAction } = require('./memory-actions');
        const result = executeAction(db, { action: args.action, target: args.target, content: args.content || null, workspace });
        return { content: [{ type: 'text', text: `${args.action} ok: ${JSON.stringify(result)}` }] };
      } catch (err) {
        const { AmbiguousTargetError, TargetNotFoundError } = require('./memory-actions');
        if (err instanceof AmbiguousTargetError) {
          const lines = ['Ambiguous target — multiple chunks match within the ambiguity delta:'];
          for (const c of err.candidates) {
            lines.push(`  [${c.id}] ${c.filePath} (score ${(c.score || 0).toFixed(3)}): ${c.content.slice(0, 100)}`);
          }
          lines.push('Re-run with an explicit chunk id.');
          return { content: [{ type: 'text', text: lines.join('\n') }], isError: true };
        }
        if (err instanceof TargetNotFoundError) {
          return { content: [{ type: 'text', text: `Target not found: ${err.target}` }], isError: true };
        }
        log(`sme_memory_action error: ${err.message}`);
        return { content: [{ type: 'text', text: `Error: ${err.message}` }], isError: true };
      }
    }
  );
